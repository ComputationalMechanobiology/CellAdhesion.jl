...
function _check_k_off(tol)
  ...
  model = SlipBondModel((k_on_0 = 0.2,), (k_off_0 = 0.8, f_1e = 1.0))
  k_off_bond = k_off(model, convert(CellAdhesionFloat, 2.0))
...
function _check_force_clusters2levels_global(tol)
  ...
  v3 = Cluster(Bond.([true,true,true, false], convert(Vector{CellAdhesionFloat}, zeros(n)), repeat([model], n)), true, convert(CellAdhesionFloat, 0.0), force_string, n, l)
  c1 = Cluster([v1, v2], true, convert(CellAdhesionFloat, 0.0), force_string, convert(CellAdhesionInt, 2), l)
  c2 = Cluster([v1, v3], true, convert(CellAdhesionFloat, 0.0), force_string, convert(CellAdhesionInt, 2), l)
  c3 = Cluster([v2, v3], true, convert(CellAdhesionFloat, 0.0), force_string, convert(CellAdhesionInt, 2), l)
